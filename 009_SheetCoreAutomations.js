...
function handleSheetAutomations(e, trueOriginalValuesForTest = null) {
  ...
  try {
    ...
    const targetRange = sheet.getRange(editedRowStart, dataBlockStartCol, numEditedRows, numColsInDataBlock);
    const finalValuesToWrite = trueOriginalValuesForTest ? JSON.parse(JSON.stringify(trueOriginalValuesForTest)) : targetRange.getValues();
    ...
    for (let i = 0; i < numEditedRows; i++) {
      ...
      if (!isSingleCellEdit) { 
        ...
        Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Detected Paste operation. Sanitizing calculated/approval fields.`);
        const fieldsToWipe = [
          combinedIndexes.index, combinedIndexes.lrfPreview, combinedIndexes.contractValuePreview,
          combinedIndexes.status, combinedIndexes.financeApprovedPrice, combinedIndexes.approvedBy,
          combinedIndexes.approvalDate, combinedIndexes.approverComments, combinedIndexes.approverPriceProposal
        ];
        fieldsToWipe.forEach(key => { inMemoryRowValues[combinedIndexes[key] - dataBlockStartCol] = ""; });
        inMemoryRowValues[combinedIndexes.approverAction - dataBlockStartCol] = "Choose Action";
        originalRowValues[combinedIndexes.status - dataBlockStartCol] = "";
        Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Wiped original status for paste re-evaluation.`);
      } else { 
        Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_singleCellSanitization' });
        if (CONFIG.protectedColumnIndices.includes(editedCol)) {
          Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_protectedColumnRevert' });
             ...
             Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: PROTECTED COLUMN. Reverted illegal edit on col ${editedCol} back to original value: '${e.oldValue}'.`);
        }
      }
      Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Data AFTER sanitization: ${JSON.stringify(inMemoryRowValues)}`);
      ...
      Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Is approval action? ${isApprovalAction}.`);
      if (isApprovalAction) {
        Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_approvalActionPath' });
        Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Detected approver action. Calling processSingleApprovalAction.`);
        ...
        Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_statusUpdatePath' });
        Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Not an approval action. Calling status determination logic.`);
        ...
        Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Status determination complete. Initial='${initialStatus}', New='${newStatus}'.`);

        if (newStatus !== initialStatus) {
            Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_statusDidChange' });
            Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Status changed. Applying updates.`);
            ...
            if (newStatus === null) { 
                Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_clearStatusOnly' });
          ...
          Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_statusUnchanged' });
          Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Status did not change. No updates needed.`);
        }
      }
      ...
      const modelName = inMemoryRowValues[combinedIndexes.model - dataBlockStartCol];
      if (modelName && !inMemoryRowValues[combinedIndexes.index - dataBlockStartCol]) {
        Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_setNewIndex' });
        ...
        Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Initialized with new Index ${inMemoryRowValues[combinedIndexes.index - dataBlockStartCol]}.`);
      }
      if (modelName && !inMemoryRowValues[combinedIndexes.approverAction - dataBlockStartCol]) {
        Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_setDefaultAction' });
        inMemoryRowValues[combinedIndexes.approverAction - dataBlockStartCol] = "Choose Action";
        Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: Initialized with default Approver Action 'Choose Action'.`);
      }
      Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Row ${currentRowNumInSheet}: FINAL in-memory data before write: ${JSON.stringify(inMemoryRowValues)}`);
    ...
    if (trueOriginalValuesForTest !== null) {
        Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Data was injected by test. Proceeding to write results to sheet.`);
    }
    targetRange.setValues(finalValuesToWrite);
    Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Wrote final values back to range ${targetRange.getA1Notation()}.`);
    ...
    if (CONFIG.featureFlags.highlightBundlesWithBorders && isSingleCellEdit) {
      ...
      if (integrityCols.includes(editedCol)) {
        ...
        Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Integrity column ${editedCol} was edited. Running metadata-based bundle check.`);
        ...
        if (oldBundleInfo && (!currentBundleNum || String(currentBundleNum) !== String(oldBundleInfo.bundleId))) {
          ...
          Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Bundle #${oldBundleInfo.bundleId} has been broken. Clearing old metadata and borders.`);
        ...
        if (currentBundleNum) {
            ...
            if (validationResult.isValid) {
              ...
              if (validationResult.startRow && validationResult.endRow && validationResult.startRow !== validationResult.endRow) {
                Log.TestCoverage_gs({ file: sourceFile, coverage: 'handleSheetAutomations_bundleValid_isMultiRow' });
                ...
                if (JSON.stringify(oldBundleInfo) !== JSON.stringify(newBundleInfo)) {
                  Log.TestCoverage_gs({ file: sourceFile, coverage: 'handleSheetAutomations_bundleStructureChanged' });
                  Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Bundle structure changed. Updating metadata and borders.`);
                  ...
                  Log.TestCoverage_gs({ file: sourceFile, coverage: 'handleSheetAutomations_bundleStructureUnchanged' });
                  Log[sourceFile](`[${sourceFile} - handleSheetAutomations] Bundle structure is unchanged. Skipping metadata write and format.`);
                }

              }
            } else { // Validation failed for the new number
              Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'handleSheetAutomations_bundleInvalid' });
...
*/
function updateCalculationsForRow(sheet, rowNum, rowValues, colIndexes, approvalWorkflowConfig, dataBlockStartCol) {
  const sourceFile = "SheetCoreAutomations_gs";
  ...
  if (approvedStatuses.includes(status)) {
    Log.TestCoverage_gs({ file: sourceFile, coverage: 'updateCalculationsForRow_getPrice_approved' });
    rentalPrice = getNumericValue(rowValues[colIndexes.financeApprovedPrice - dataBlockStartCol]);
  } else {
    Log.TestCoverage_gs({ file: sourceFile, coverage: 'updateCalculationsForRow_getPrice_pending' });
  ...
  // --- THIS IS THE FIX ---
  const chosenCapex = getNumericValue(rowValues[colIndexes.aeCapex - dataBlockStartCol]);
  Log[sourceFile](`[${sourceFile} - updateCalculationsForRow] Row ${rowNum}: Inputs: rentalPrice=${rentalPrice}, chosenCapex=${chosenCapex}`);
  // --- END FIX ---
  ...
  if (rentalPrice === 0 && (!chosenCapex || chosenCapex === 0)) {
    Log.TestCoverage_gs({ file: sourceFile, coverage: 'updateCalculationsForRow_clearValues' });
    ...
    Log.TestCoverage_gs({ file: sourceFile, coverage: 'updateCalculationsForRow_calculateValues' });
    if (!chosenCapex || chosenCapex <= 0) {
      Log.TestCoverage_gs({ file: sourceFile, coverage: 'updateCalculationsForRow_missingCapex' });
      // --- THIS IS THE FIX ---
      ...
      if (chosenCapex > 0 && rentalPrice > 0 && term > 0) {
        Log.TestCoverage_gs({ file: sourceFile, coverage: 'updateCalculationsForRow_lrfSuccess' });
        rowValues[colIndexes.lrfPreview - dataBlockStartCol] = (rentalPrice * term) / chosenCapex;
      } else {
        Log.TestCoverage_gs({ file: sourceFile, coverage: 'updateCalculationsForRow_lrfZero' });
        rowValues[colIndexes.lrfPreview - dataBlockStartCol] = "";
      }
    }
    ...
    if (rentalPrice > 0 && term > 0 && quantity > 0) {
      Log.TestCoverage_gs({ file: sourceFile, coverage: 'updateCalculationsForRow_cvSuccess' });
      rowValues[colIndexes.contractValuePreview - dataBlockStartCol] = rentalPrice * term * quantity;
    } else {
      Log.TestCoverage_gs({ file: sourceFile, coverage: 'updateCalculationsForRow_cvZero' });
      rowValues[colIndexes.contractValuePreview - dataBlockStartCol] = "";
    }
  }
  ...
  Log.TestCoverage_gs({ file: 'SheetCoreAutomations.gs', coverage: 'updateCalculationsForRow_end' });
