...
var _originalLogTableActivity =
  typeof logTableActivity !== "undefined" ? logTableActivity : null;
var _originalLogGeneralActivity =
  typeof logGeneralActivity !== "undefined" ? logGeneralActivity : null;

TestMocks.MOCK_TOAST_MESSAGE =
  TestMocks.MOCK_TOAST_MESSAGE === undefined
    ? null
    : TestMocks.MOCK_TOAST_MESSAGE;
TestMocks.MOCK_SESSION_EMAIL =
  TestMocks.MOCK_SESSION_EMAIL === undefined
    ? "test.approver@example.com"
    : TestMocks.MOCK_SESSION_EMAIL;
TestMocks.LOG_TABLE_ACTIVITY_CALLS =
  TestMocks.LOG_TABLE_ACTIVITY_CALLS === undefined
    ? []
    : TestMocks.LOG_TABLE_ACTIVITY_CALLS;
TestMocks.LOG_GENERAL_ACTIVITY_CALLS =
  TestMocks.LOG_GENERAL_ACTIVITY_CALLS === undefined
    ? []
    : TestMocks.LOG_GENERAL_ACTIVITY_CALLS;
TestMocks.MOCK_SHEET = {
  getName: function() {
    return "MockUnitTestSheet";
  },
};
...
function setUp() {
  ...
  logTableActivity = function(options) {
    TestMocks.LOG_TABLE_ACTIVITY_CALLS.push(options);
  };
  logGeneralActivity = function(options) {
    TestMocks.LOG_GENERAL_ACTIVITY_CALLS.push(options);
  };
...
function tearDown() {
  if (TestMocks.ORIGINAL_SS_APP_GET_ACTIVE) {
    SpreadsheetApp.getActive = TestMocks.ORIGINAL_SS_APP_GET_ACTIVE;
  }
  if (_originalLogTableActivity) logTableActivity = _originalLogTableActivity;
  if (_originalLogGeneralActivity)
    logGeneralActivity = _originalLogGeneralActivity;
...
function runApprovalWorkflow_UnitTests() {
  Log.TestResults_gs(
    "--- Starting ApprovalWorkflow Unit Test Suite (REFACTORED) ---"
  );
...
function runApprovalWorkflow_IntegrationTests() {
  Log.TestResults_gs(
    "--- Starting ApprovalWorkflow Integration Test Suite (REFACTORED) ---"
  );
  ...
  Log.TestResults_gs(
    "--- ApprovalWorkflow Integration Test Suite Finished ---"
  );
}
...
function _createMockEditEvent(row, column, value, oldValue) {
  return {
    range: { getRow: () => row, getColumn: () => column },
    value: value,
    oldValue: oldValue,
  };
}
...
function _getCleanRow(templateName) {
  ...
  if (!templateRow)
    throw new Error(`Mock data template "${templateName}" not found.`);
...
function test_processSingleApprovalAction_approveOriginalPrice() {
  setUp();
  const testName =
    "Unit Test: processSingleApprovalAction - Approve Original Price";
  ...
  const mockEvent = _createMockEditEvent(
    10,
    colIndexes.approverAction,
    "Approve Original Price",
    "Choose Action"
  );
  const result = processSingleApprovalAction(
    TestMocks.MOCK_SHEET,
    10,
    mockEvent,
    inMemoryRow,
  ...
  _assertEqual(
    inMemoryRow[colIndexes.status - 1],
    CONFIG.approvalWorkflow.statusStrings.approvedOriginal,
    `${testName} - Status should be Approved (Original Price)`
  );
  _assertEqual(
    inMemoryRow[colIndexes.financeApprovedPrice - 1],
    100,
    `${testName} - Finance Approved Price should be original price`
  );
  _assertEqual(
    inMemoryRow[colIndexes.approvedBy - 1],
    TestMocks.MOCK_SESSION_EMAIL,
    `${testName} - Approved By should be set`
  );
  _assertNotNull(
    inMemoryRow[colIndexes.approvalDate - 1],
    `${testName} - Approval Date should be set`
  );
  tearDown();
}
...
function test_processSingleApprovalAction_approveNewPrice() {
  ...
  _assertEqual(
    ...
    `${testName} - Status should be Approved (New Price)`
  );
  _assertEqual(
    inMemoryRow[colIndexes.financeApprovedPrice - 1],
    90,
    `${testName} - Finance Approved Price should be proposed price`
  );
  tearDown();
}
...
function test_processSingleApprovalAction_rejectWithComment() {
  setUp();
  const testName =
    "Unit Test: processSingleApprovalAction - Reject with Comment";
  const colIndexes = MOCK_DATA_UNIT.approvalWorkflowTestColIndexes;
  ...
  _assertEqual(
    ...
    `${testName} - Status should be Rejected`
  );
  _assertEqual(
    inMemoryRow[colIndexes.financeApprovedPrice - 1],
    "",
    `${testName} - Finance Approved Price should remain empty`
  );
  tearDown();
}
...
function test_processSingleApprovalAction_fail_InvalidStatus() {
  setUp();
  const testName =
    "Unit Test: processSingleApprovalAction - Fail on Invalid Status";
  const colIndexes = MOCK_DATA_UNIT.approvalWorkflowTestColIndexes;
  ...
  _assertEqual(
    ...
    `${testName} - Status should remain Draft`
  );
  _assertEqual(
    inMemoryRow[colIndexes.approverAction - 1],
    "Choose Action",
    `${testName} - Action should be reverted`
  );
  _assertNotNull(
    TestMocks.MOCK_TOAST_MESSAGE,
    `${testName} - A toast message should be shown`
  );
  _assertTrue(
    TestMocks.MOCK_TOAST_MESSAGE.includes("cannot be processed"),
    `${testName} - Toast should explain the error`
  );
  tearDown();
}
...
function test_processSingleApprovalAction_fail_ApproveOriginalNoPrice() {
  setUp();
  const testName =
    "Unit Test: processSingleApprovalAction - Fail Approve Original Price with no price";
  const colIndexes = MOCK_DATA_UNIT.approvalWorkflowTestColIndexes;
  ...
  _assertTrue(
    TestMocks.MOCK_TOAST_MESSAGE.includes("invalid or missing LRF"),
    `${testName} - Toast should explain the error`
  );
  tearDown();
}
...
function test_processSingleApprovalAction_fail_ApproveNewNoPrice() {
    ...
    const colIndexes = MOCK_DATA_UNIT.approvalWorkflowTestColIndexes;
    const originalRow = _getCleanRow('pendingApprovedOriginal');
    // Manually blank out the approver price proposal in our 21-column mock data
...
function test_processSingleApprovalAction_fail_RejectNoComment() {
  setUp();
  const testName =
    "Unit Test: processSingleApprovalAction - Fail Reject with no comment";
  const colIndexes = MOCK_DATA_UNIT.approvalWorkflowTestColIndexes;
  ...
  _assertTrue(
    TestMocks.MOCK_TOAST_MESSAGE.includes("without adding a comment"),
    `${testName} - Toast should explain the error`
  );
  tearDown();
}
...
function test_processSingleApprovalAction_fail_InvalidLRF() {
  setUp();
  const testName =
    "Unit Test: processSingleApprovalAction - Fail Approve with invalid LRF";
  const colIndexes = MOCK_DATA_UNIT.approvalWorkflowTestColIndexes;
  const originalRow = _getCleanRow("pendingApprovedOriginal");
...
function test_handleSheetAutomations_directApprovalAction_Integration() {
    ...
    withTestConfig(function() {
      ...
      withTestSheet(MOCK_DATA_INTEGRATION.csvForApprovalWorkflowTests, function(sheet) {
          const targetRow = 2; 
          // Use the live CONFIG object which has the correct 21-column indices
...
function test_runSheetHealthCheck_findsAndFixesInconsistencies() {
  const testName =
    "Integration Test: runSheetHealthCheck finds and fixes inconsistencies";
  withTestConfig(function() {
    setUp();
    withTestSheet(MOCK_DATA_INTEGRATION.csvForHealthCheckTests, function(
      sheet
    ) {
      ...
      _assertEqual(
        ...
        `${testName} - R2 (Approved without date) should be reverted to Pending.`
      );
      _assertEqual(
        status_R3,
        CONFIG.approvalWorkflow.statusStrings.approvedOriginal,
        `${testName} - R3 (Healthy Approved row) should remain Approved.`
      );
      _assertEqual(
        status_R4,
        CONFIG.approvalWorkflow.statusStrings.pending,
        `${testName} - R4 (Rejected without date) should be reverted to Pending.`
      );
      _assertTrue(
        TestMocks.MOCK_TOAST_MESSAGE.includes("Reverted 2 rows"),
        `${testName} - Toast should report 2 fixes.`
      );
    });
    tearDown();
  });
...
function test_runSheetHealthCheck_noInconsistenciesFound() {
    ...
    // Use the live CONFIG object to get the correct 21-column index for approvalDate
